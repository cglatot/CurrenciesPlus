    ...
    private void HydratePopupMenuInternal()
    {
        ...
        foreach (Currency currency in group0Currencies) {
            ...
            if (currency.Type == CurrencyType.Maelstrom || currency.Type == CurrencyType.TwinAdder || currency.Type == CurrencyType.ImmortalFlames) {
                ...
                else if (GetActualAmount(currency.Type) >= 75000) setTextColor = new(0xFF52ABFF);
            }
        ...
        foreach (Currency currency in Currencies.Values) {
            ...
            if (currency.GroupId == 1) {
                ...
                else if (GetActualAmount(currency.Type) >= 3000) setTextColor = new(0xFF52ABFF);
            }
            else if (currency.GroupId == 2) {
                if (GetActualAmount(currency.Type) >= 2000) setTextColor = new(0xFF6565fc);
                else if (GetActualAmount(currency.Type) >= 1500) setTextColor = new(0xFF52ABFF);
            }
            else if (currency.GroupId == 3) {
                if (GetActualAmount(currency.Type) >= 20000) setTextColor = new(0xFF6565fc);
                else if (GetActualAmount(currency.Type) >= 15000) setTextColor = new(0xFF52ABFF);
            }
                ...
                else if (GetActualAmount(currency.Type) >= 1200) setTextColor = new(0xFF52ABFF);
            }
